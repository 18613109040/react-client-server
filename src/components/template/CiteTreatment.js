...
import {restTreatment,requestTreament} from "../../store/actions/Treatment";
...
class CiteTreatment extends Component {
  static defaultProps={
		title:"引用【治疗理疗】模板",
  ...
  componentWillReceiveProps(nextProps){
  	...
  	if(nextProps.overAllVisble!= this.props.overAllVisble){
  		...
  		this.setState({
  			visibleCiteTreatMent:nextProps.overAllVisble,
  ...
  overAllOk(){
  	...
  	if(this.state.selectData.length>0)
  		this.props.dispatch(restTreatment(this.state.selectData));
  			...
  			this.props.dispatch(queryRecipeTempletDetail(getData,(res)=>{
  				if(res.status == '0'){
  					...
  					for(let i=0;i<res.data.list.length;i++){
		  				num.push(i);
		  				let shop_no = getUser().shop_no;
		  				this.props.dispatch(requestTreament({
					      pharmacy_type: 32,
					      shop_no: shop_no,
					      keyword: res.data.list[i].item_name
					    },(respose)=>{
					    	if(respose.status=='0'){
					    		if(respose.data.length>0){
					    			Object.assign(res.data.list[i],{no:res.data.list[i].quantity},{standard:res.data.list[i].unit},{check:true},{item_price:respose.data[0].price});
					    		}
					    	}
  ...
  render(){
  	...
  	const columns = [{
	    ...
	    <span>
	     	...
	     	<span>({`${record.acupoint_name4}`})</span>
	    </span>
	  )
    ...
    return(
      <div className="prescriptionTemplate">
      	<Modal
      		...
      		maskClosable={false}
      		visible={this.state.visibleCiteTreatMent}
...
export default connect(mapStateToProps)(CiteTreatment)
