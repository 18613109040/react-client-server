...
class SaveCheckMed extends Component {
  static defaultProps={
		title:"【检验检查】另存为模板",
		dealValue:""
	};
  ...
  componentWillReceiveProps(nextProps){
  	...
  	if(nextProps.saveVisble!= this.props.saveVisble){
  		console.dir(nextProps.checkRecipeList[this.props.checkId-1])
  		let num = [];
  		for(let i=0;i<nextProps.checkRecipeList[this.props.checkId-1].data.filter(item => item.check == true).length;i++){
	  		num.push(i)
	  	}
  		console.dir(num)
  		this.setState({
  			visible:nextProps.saveVisble,
  			saveData:nextProps.checkRecipeList[this.props.checkId-1].data.filter(item => item.check == true),
  			selectData:nextProps.checkRecipeList[this.props.checkId-1].data.filter(item => item.check == true),
  			selectAll:num
  		})
          				...
          				<TreeNode 
          					title={
	          					<div className="addtemple">
		          					<span className="title-temp">检验检查</span>
		          					<span className="icon-hiden">
...
SaveCheckMed = Form.create({})(SaveCheckMed);
export default connect(mapStateToProps)(SaveCheckMed)
