...
class UsePreAllZXTemplate extends Component {
  constructor(props){
    super(props);
    this.state = {
      dataSource:[]
  ...
  clickSaveSome(){
    const {wholePlate,zXPrescriptionInfo} = this.props;
    const {items} = zXPrescriptionInfo[wholePlate.activeKey];
    const _self = this;
    //判断处方是否已有数据
    if(items.length > 1){
      confirm({
        ...
        onOk() {
          ...
          _self.sendMedicineData(dataSourceSome,zXPrescriptionInfo[wholePlate.activeKey],wholePlate.activeKey,_self.props.buttonClick());
        },
        onCancel() {
    ...
    if(items.length > 1){
      confirm({
        ...
        onOk() {
          ...
          _self.sendMedicineData(dataSource,zXPrescriptionInfo[wholePlate.activeKey],wholePlate.activeKey,_self.props.buttonClick());
        },
        onCancel() {
          _self.props.buttonClick();
        },
...
UsePreAllZXTemplate.contextTypes={
  router: React.PropTypes.object.isRequired
};
UsePreAllZXTemplate.defaultProps={
  wholePlate:{},
  buttonClick:()=>{}
};
UsePreAllZXTemplate.propTypes = {
...
export default connect(mapStateToProps)(UsePreAllZXTemplate)
