...
class ZYMedicineTemplate extends Component {
  static defaultProps={
		title:"【中药饮片】另存为模板",
		dealValue:""
	};
  ...
  feachData(){
  	...
  	this.props.dispatch(getQueryRecipeTempletTree(postData,(res)=>{
  		...
  		if(res.status.toString()=="0"){
			...
			// 					page_no:'1',
			// 					page_size:"100",
			// 					parent_id:list[index].templet_id,
			// 					staff_id:getUser().doctor_id
			// 				}
			// 				this.props.dispatch(getQueryRecipeTempletTree(postData,(res)=>{
			// 					const {citeData} = this.state;
			// 					citeData[index].list = res.data.list;
			// 					this.setState({
			// 						citeData:[...citeData]
			// 					})
			// 				}))
  ...
  componentWillReceiveProps(nextProps){
  	if(nextProps.saveVisble!= this.props.saveVisble){
			let medicineInfo = nextProps.checkRecipeList[this.props.checkId].items;
	    ...
	    medicineInfo = medicineInfo.filter(item=>item.item_name !='膏方制作费（300）');
  		let num = [];
  		for(let i=0;i<medicineInfo.length - 1;i++){
	  		num.push(i)
	  	}
  		this.setState({
  			visible:nextProps.saveVisble,
  			saveData:medicineInfo.slice(0,-1),
  			selectData:medicineInfo.slice(0,-1),
  			selectAll:num
  		})
  ...
  onSave(e){
  	...
  	const This = this ;
		...
		}else if(/^\d+-\d+$/.test(this.state.selectId.toString())){
			Modal.error({
				title: '模板添加错误',
				content: '另存为模板不能添加在模板详情上',
			});
		}else{
							...
							Modal.confirm({
								...
								onOk(){
									...
									const saveData = {
										...
										templet_type:'4',//4 中药饮片, 5 中药饮片 6检验检查, 7治疗理疗
										list:temp
									}
  ...
  render(){
    ...
    const formItemLayout = {
      ...
      wrapperCol: { span: 19 },
		...
		// const loopTreeLeaf = data => data.map((item) => {
		// 	return <TreeNode
		// 						disabled={true}
		// 						key={item.templet_id+"-0"}
		// 						name={item.templet_name}
		// 						title={item.templet_name}
		// 					/>
		// })
      ...
      return <TreeNode
      				...
      				isLeaf={false}>
								{/* {loopTreeLeaf(item.list?item.list:[])} */}
							</TreeNode>;
    });
    return(
          				...
          				<TreeNode
          					title={
	          					<div className="addtemple">
		          					<span className="title-temp">中药饮片</span>
		          					<span className="icon-hiden">
...
ZYMedicineTemplate.contextTypes={
  router: React.PropTypes.object.isRequired
};

function mapStateToProps(state){
  return {
    checkRecipeList: state.prescriptionInfo,     // 列表
  }
}
ZYMedicineTemplate = Form.create({})(ZYMedicineTemplate);
export default connect(mapStateToProps)(ZYMedicineTemplate)
